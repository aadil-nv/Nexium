...
import {
  ...
  FaCheckCircle,
  FaTimesCircle,
} from "react-icons/fa"; // Import icons for verified/unverified
...
const ManagerCard: React.FC<IManagerCardProps> = ({
  ...
  isVerified,
  isBlocked,
  managerId, // Assuming `managerId` is passed as a prop
}) => {
  ...
  const handleToggleStatus = async () => {
    try {
      ...
      const response = await businessOwnerInstance.patch("/businessOwner/api/manager/block-manager", {
        managerId,
        isBlocked: !isBlocked,
      });
      ...
      if (response.data.success) {
        alert(
          `Manager successfully ${isBlocked ? "unblocked" : "blocked"}!`
        );
      ...
      <div className="p-4">
        ...
        <div className="mt-4 flex flex-col sm:flex-row md:flex-row justify-center gap-2">
          ...
          <motion.button
            ...
            className={`text-white px-4 py-2 rounded transition duration-300 w-full sm:w-auto md:w-auto flex items-center justify-center gap-2 text-sm ${
              isBlocked
