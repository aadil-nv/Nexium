...
import { z } from 'zod';
import { FaArrowLeft, FaEye, FaEyeSlash, FaCheckCircle } from 'react-icons/fa'; // Added FaCheckCircle for success icon
...
export default function NewPassword() {
    ...
    const [showSuccessModal, setShowSuccessModal] = useState(false); // State to show the success modal
    ...
    const handleChangePassword = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            const response = await axios.patch('http://localhost:7000/api/business-owner/add-newpassword', {
                password,
                email,
            });
        ...
        <div className={`w-full h-screen flex items-center justify-center ${theme === 'dark' ? 'bg-gray-900' : 'bg-white'}`}>
                ...
                <div className="fixed inset-0 bg-black bg-opacity-60 flex items-center justify-center z-50">
                    <div className={`p-6 rounded-lg shadow-lg transform transition-transform duration-300 ease-out scale-100 animate-fade-in ${theme === 'dark' ? 'bg-gray-800 text-white' : 'bg-white text-gray-800'}`}>
                        <div className="flex flex-col items-center">
                            ...
                            {/* Optional: Add a subtle message or note */}
            ...
            <div className={`p-8 rounded-lg w-[90%] max-w-md transition-shadow duration-300 ${theme === 'dark' ? 'bg-gray-800 shadow-lg shadow-blue-500' : 'bg-white shadow-md shadow-gray-200'}`}>
                ...
                <form onSubmit={handleChangePassword}>
                    ...
                    <div className="mb-4 relative">
                        <label className={`block ${theme === 'dark' ? 'text-white' : 'text-gray-700'}`} htmlFor="confirmPassword">
                            Confirm Password
                        </label>
                        <input
