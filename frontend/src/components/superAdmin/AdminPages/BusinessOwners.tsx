...
// Define the Subscription and Company interfaces
...
const BusinessOwners: React.FC = () => {
  ...
  // Fetch business owners data from the backend
  useEffect(() => {
    const fetchBusinessOwners = async () => {
      ...
      try {
        ...
        const businessOwners = response.data.businessOwners.map((owner: any) => ({
          id: owner._id, // Assuming _id is used for the id
          name: owner.name,
          email: owner.email,
          phone: owner.phone,
          registrationNumber: owner.registrationNumber,
          subscription: {
            planName: owner.subscription?.planName ?? "N/A",
            planType: owner.subscription?.planType ?? "N/A",
            startDate: owner.subscription?.startDate ?? "N/A",
            endDate: owner.subscription?.endDate ?? "N/A",
            status: owner.subscription?.status ?? "N/A",
          },
        }));
  ...
  const handleViewDetails = (owner: Company) => {
    alert(`Details of ${owner.name}: \nEmail: ${owner.email} \nPhone: ${owner.phone} \nRegistration Number: ${owner.registrationNumber}`);
  };

  const exportToExcel = () => {
    const worksheet = XLSX.utils.json_to_sheet(data);
    const workbook = XLSX.utils.book_new();
    XLSX.utils.book_append_sheet(workbook, worksheet, "businessOwners");
    XLSX.writeFile(workbook, "businessOwners.xlsx");
  };
    ...
    <div className="p-5 max-w-6xl mx-auto bg-white text-gray-800 rounded-lg shadow-lg">
      <div className="flex flex-col lg:flex-row justify-between items-center mb-4 space-y-4 lg:space-y-0 lg:space-x-4">
        ...
        <div className="flex flex-wrap space-x-3 justify-center">
          <CSVLink data={data} filename={"businessOwners.csv"}>
