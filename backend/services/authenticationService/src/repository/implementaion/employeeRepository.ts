...
export default class EmployeeRepository
  extends BaseRepository<IEmployeeDocument>
  ...
  async findByCredentialEmail(email: string): Promise<IEmployeeDocument> {
    try {
      const employee = await this._employeeModel.findOne({
        "employeeCredentials.companyEmail": email,
      });
      return employee;
    } catch (error) {
  ...
  async updateVerificationStatus(email: string,businessOwnerId: ObjectId): Promise<any> {
    try {
      const _switchDb = mongoose.connection.useDb(businessOwnerId.toString(), {
        useCache: true,
      });
      ...
      return await this._employeeModel
        .updateOne({ "personalDetails.email": email }, { isVerified: true })
        .exec();
    } catch (error) {
  ...
  async updateOtp(email: string, otp: string): Promise<void> {
    ...
    if (result.modifiedCount === 0) {
      throw new Error(
        "Failed to update OTP. No document found or OTP was not changed."
      );
    }
  }
  ...
  async updateEmployee(employee: any): Promise<any> {
    try {
      ...
      return this._employeeModel.updateOne(
        { _id: employeeId },
        { $set: updateData }
      );
    } catch (error) {
  ...
  async updateIsActive(bsuinessOwnerId: string,id: any,isActive: boolean ): Promise<IEmployeeDocument | null> {
    try {
      const _switchDb = mongoose.connection.useDb(bsuinessOwnerId, {
        useCache: true,
      });
      const switchedDB = _switchDb.model(
        "employees",
        this._employeeModel.schema
      );
      const employee = await this._employeeModel
